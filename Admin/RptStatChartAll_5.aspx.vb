...
Partial Class Admin_RptStatChartAll_5
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Page.IsPostBack = False Then
            ...
            If Request.QueryString("rt") = 1 Then
                lblHead.Text = "ข้อมูลสำนักงานขนส่ง (รถประจำถิ่น)" ' "จำนวนรถประจำถิ่นที่ขออนุญาตเข้ามาในราชอาณาจักรเป็นการชั่วคราว <br />จำแนกตามผู้ประกอบการธุรกิจนำเที่ยว (ผู้ขอยื่น)" '
            ElseIf Request.QueryString("rt") = 2 Then
                lblHead.Text = "ข้อมูลสำนักงานขนส่ง (รถท่องเที่ยว)" ' "จำนวนรถท่องเที่ยวที่ขออนุญาตเข้ามาในราชอาณาจักรเป็นการชั่วคราว <br />จำแนกตามผู้ประกอบการธุรกิจนำเที่ยว (ผู้ขอยื่น)" '
            End If
            loadChart()
        End If
    End Sub
    ...
    Private Sub loadChart()
        ...
        Dim table_2 As Table = PopulateChart.AddTable(Page, 11, typeuser_id, _year, lblHead, txtsdate, txtedate, 0, ddldate.SelectedValue)
        If IsMobile Then
            table_2.Width = Unit.Pixel(1024)
        ...
        tr = New TableRow
        tc = New TableCell
        Dim divChart12 As String = "<br /><br /><div id='container12' " & styleChart & " ></div> <br /><br />"
        tc.BorderColor = Drawing.Color.DarkGray
        ...
        _data = PopulateChart.getDataChart(Page, "coalesce(type_car.type_name_th,'ไม่ระบุ') as car_type , coalesce(admin.admin_name,'ไม่ระบุ') ", 12, typeuser_id, _year, txtsdate, txtedate, 0, ddldate.SelectedValue)
        pScriptAddChart = pScriptAddChart & _data & " AddChart12(_data,_categories,'container12',true,'จำนวนรถที่ได้รับอนุญาตฯ จำแนกตามสำนักงาน และประเภทรถ'); "
    ...
    Private Function LoadDataExport(ByVal IsExcel As Boolean) As DataTable
        ...
        Dim dtChart As DataTable = PopulateChart.ConvertDataExport(Page, 11, Request.QueryString("rt"), _year, lblHead, txtsdate, txtedate, _limit, IsExcel, ddldate.SelectedValue)
        Return dtChart
    End Function
    ...
    Protected Sub BtnDownloadCSV_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnDownloadCSV.Click
        ...
        Response.AddHeader("Content-Disposition", "attachment;filename=myChart" & 11 & ".csv")
    ...
    Protected Sub BtnDownloadExcel_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnDownloadExcel.Click
        ...
        Response.AddHeader("content-disposition", "attachment;filename=myChart" & 11 & ".xls")
        Response.Charset = ""
        ...
        headerTable = "<table border=1><tr><td colspan=" & cntColumns & "><center><b>การได้รับอนุญาตรถเข้ามาในราชอาณาจักร<b></center></td></tr><tr>" & _
