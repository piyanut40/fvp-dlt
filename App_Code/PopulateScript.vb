...
Public Class PopulateScript
    ...
    Public Sub SetGrid_Footable(ByVal pGrid As GridView, ByVal dt As DataTable)
        ...
        If pGrid Is Nothing Then
            Throw New ArgumentNullException("pGrid", "pGrid ไม่สามารถเป็น Nothing ได้")
        End If

        If dt Is Nothing Then
            Throw New ArgumentNullException("dt", "dt ไม่สามารถเป็น Nothing ได้")
        End If
        ...
        Try
            ' กำหนดค่าให้ GridView
            pGrid.DataSource = dt
            pGrid.DataBind()

            ' ตรวจสอบว่า Grid มีแถว Header หรือไม่ก่อนเข้าถึง
            If pGrid.HeaderRow IsNot Nothing Then
                pGrid.Attributes.Add("class", "table")
                pGrid.HeaderRow.TableSection = TableRowSection.TableHeader
                pGrid.GridLines = GridLines.None
            End If
        Catch ex As Exception
            ' ถ้า session หลุด ให้ redirect ไปหน้า login
            ...
            Else
                ' แสดงข้อผิดพลาด
                Throw New Exception("เกิดข้อผิดพลาดขณะกำหนดค่า GridView: " & ex.Message)
            End If
        End Try
    End Sub
