    ...
    Private Sub loadData()
        ...
        Try
            ...
            If Request.QueryString("rt") <> 5 And Request.QueryString("rt") <> 2 Then
                If Request.QueryString("rt") = 1 Then
                    strsql = " SELECT  Row_number() over (order by regis_date desc nulls last) as number ,* FROM  " & _
                       ...
                       " , CAST(case when user_prename = 'Other' then '' else user_prename end || ' ' || user_travel_group.user_name || ' ' || user_lastname as varchar) as guide  , name_company , travel_group.start_date , travel_group.exp_date, " & _
                       " (select count(*) from user_travel_group as t1  " & _
                       " LEFT JOIN travel_group as t2 on t2.group_id = t1.group_id  " & _
                       " WHERE t1.idcard like user_travel_group.idcard  and (start_date >= travel_group.start_date AND exp_date <= travel_group.exp_date) )as count " & _
                       " ,travel_group.start_date as start_date1,travel_group.exp_date as exp_date1" & _
                       " FROM license " & _
                       " LEFT JOIN driver on driver.driver_id = license.driver_id  LEFT JOIN car on car.car_id = license.car_id " & _
                       " LEFT JOIN type_car on car.typecar_id = type_car.type_id  LEFT JOIN status on license.status_id = status.status_id " & _
                       ...
                       " LEFT JOIN user_travel_group on user_travel_group.group_id = travel_group_car.group_id " & _
            ...
            ElseIf Request.QueryString("rt") = 2 Then
                strsql = " SELECT  Row_number() over (order by regis_date desc nulls last) as number ,* FROM  " & _
                        " ( SELECT plate , country_car , license.license_id , token , " & admin_id & " admin_id , CAST(case when driver.prename = 'Other' then '' else driver.prename end || ' ' || name || ' ' || surname as varchar) as name, '' as vehicle_type  , " & strType & " as typecar_en " & _
                        " , brands, model as models, case when (check_tab0 + check_tab1 + check_tab2 + check_tab3 + check_tab4 + check_tab8) > 6 then 'รออนุมัติ(แก้ไขคำขอ)' else status_th end as status_th ,  " & _
                      " CAST('LicenseApp.aspx?rt=2&token=' || token as varchar ) as urlDoc, '' " & _
                      " as urlMgt, typeuser_id, license.status_id,regis_date  , check_tab1, check_tab2, check_tab3, check_tab4, check_tab5, check_tab6, check_tab7 , check_tab0 , check_tab8, reason_app as reason   , " & _
                      ...
                      "name_company , to_char(travel_group.start_date , 'DD-MM-YYYY') as start_date , " & _
                      " to_char(travel_group.exp_date , 'DD-MM-YYYY') as exp_date ,travel_group.start_date as start_date1,travel_group.exp_date as exp_date1" & _
                      " FROM license  LEFT JOIN driver on driver.driver_id = " & _
                       " license.driver_id  LEFT JOIN car on car.car_id = license.car_id  LEFT JOIN type_car on car.typecar_id = type_car.type_id " & _
                       "  LEFT JOIN status on license.status_id = status.status_id  LEFT JOIN user_travel on user_travel.user_id =  license.travel_id  " & _
                        " LEFT JOIN travel_group_car on travel_group_car.license_id = license.license_id  " & _
                        " LEFT JOIN travel_group on travel_group.group_id = travel_group_car.group_id " & _
                        " WHERE typeuser_id = 2 and license.status_id not in (4, 2, 5 , 3 , 1 , 7 , 8 , 9)  ) as dt " 'and license.status_id not in (4, 5 , 3 , 1 , 7)  ) as dt "

            Else
                strsql = "SELECT  Row_number() over (order by regis_date desc nulls last) as number ,* FROM (SELECT  registration_no as plate ,  country_car , license_id , token , transport_operator_name as name, vehicle_type " & _
                       " , vehicle_category as typecar_en, brand as brands, model as models, status_th , " & urlLicenseApp & "&token=' || token as varchar ) as urlDoc,license.regis_date " & _
                       " , CAST('CarCommerceMgt.aspx?token=' || token as varchar ) as urlMgt, typeuser_id, license.status_id " & _
        ...
        Try
            ...
            Else
                ...
                End If
                    ...
                    Dim lnkDel As HyperLink = e.Row.Cells(14).FindControl("HyperLinkDel")
    ...
    Protected Sub BtnSave_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnSave.Click
        ...
        Try
            ...
            Else
                ...
                Dim minval As Date = dbConnect.executeScalar(" select act_ends from act " & _
                                     " LEFT JOIN license on license.act_id = act.act_id " & _
            ...
            If ddlstatus.SelectedValue <> 0 Then
                ...
                Dim str = " SELECT driver.prename, coalesce(driver.prename_other,'')as prename_other , driver.name ,  driver.surname , idcard_no , driver.email , type_user.typename_th " & _
