...
Partial Class Admin_LicenseDtl
    ...
    Private Sub LoadData()
        ...
        Dim receipt_pc As String = ""
        Dim registrar_name As String = ""
        Dim registrar_position As String = ""
        Dim receipt_process As String = ""
        ...
        Try
            ...
            strsql = "SELECT * FROM ( SELECT driver.*, car.*, act.*, license.email as licenseemail , fname , lname, license.license_id , token , CAST(driver.prename || ' ' || driver.name || ' ' || driver.surname as varchar) as driver_name, check_tab0, check_tab1, check_tab2, check_tab3, check_tab4 , check_tab8 " & _
                   ...
                   ",  (select string_agg(prov_th , ', ' ) from area_group left join province on province.prov_code = area_group.prov_code where group_id in (select group_id from travel_group_car where license_id = license.license_id)) as provarea "
            ...
            strsql = strsql & " , receipt , receipt_date , receipt_pc , registrar_name , registrar_position , receipt_process, country_car , border_check.border_nameth , admin.admin_name "
            ...
            Else
                strsql = strsql & " , (select border_nameth from border_check WHERE border_check.border_id = license.checkout_id) as borderout_nameth  "
            End If
            strsql = strsql & ", old_group_id, attachments_file_name, attachments_file_saved, reason, travel_itinerary_filesaved " & _
            " , comments_tab0  , comments_tab1 , comments_tab2 , comments_tab3 , comments_tab4 , comments_tab5 , comments_tab6 , comments_tab7 , comments_tab8 , coalesce(dtCar.cntCar,0) as cnt_car " & _
            " FROM license " & _
            ...
            If dr.Read Then
                ...
                If Not dr("receipt_pc") Is DBNull.Value Then
                    receipt_pc = dr("receipt_pc")
                End If


                If Not dr("registrar_name") Is DBNull.Value Then
                    registrar_name = dr("registrar_name")
                End If

                If Not dr("registrar_position") Is DBNull.Value Then
                    registrar_position = dr("registrar_position")
                End If

                If Not dr("receipt_process") Is DBNull.Value Then
                    receipt_process = dr("receipt_process")
                End If
            ...
            Dim strreceipt As String = "select receipt_no , receipt_date , receipt_pc , registrar_name , registrar_position , receipt_process from receipt where license_id = " & license_id
            ...
            If receipt <> "" Then
                ...
                With nrow2
                    ...
                    .Item("receipt_pc") = receipt_pc
                    .Item("registrar_name") = registrar_name
                    .Item("registrar_position") = registrar_position
                    .Item("receipt_process") = receipt_process
                    If receipt_date <> "" Then
                        .Item("receipt_date") = receipt_date
                    End If
                End With
                dtreceipt.Rows.InsertAt(nrow2, 0)
                nrow2 = Nothing
            End If
