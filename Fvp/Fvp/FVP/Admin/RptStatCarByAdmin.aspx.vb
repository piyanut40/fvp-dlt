...
Partial Class Admin_RptStatCarByAdmin
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Page.IsPostBack = False Then
            ...
            If Request.QueryString("type") = 1 Then
                'lblHead.Text = lblHead.Text & " ประจำเดือน"
    ...
    Private Sub PopulateReport()
        Try
            ...
            rpt.ReportPath = "Report\RptStatCarByAdmin.rdlc"
            rpt.DataSources.Add(New ReportDataSource("DtStat", Dt))
            ...
            If pBrowser = "Chrome" Or pBrowser = "Firefox" Then
                ...
                ReportViewer1.Height = Unit.Pixel(800)
            Else
    ...
    Private Function getData() As DataTable
        ...
        Try
            ...
            Dim strOrder As String = "select row_number() OVER ( PARTITION BY _admin_name  ORDER BY cnt DESC) AS i , _admin_name , _border_nameth_in  from ( " & _
            " select  count(distinct car.car_id) cnt  , coalesce(admin.admin_name,'ไม่ระบุ') _admin_name, coalesce(border_checkin.border_nameth,'ไม่ระบุ') as _border_nameth_in " & _
            ...
            strWhere & " group by coalesce(admin.admin_name,'ไม่ระบุ') , coalesce(border_checkin.border_nameth,'ไม่ระบุ')  ) dt "

            Dim strOrderGroup As String = " select row_number() OVER ( ORDER BY cnt DESC) AS i , _admin_name   from ( " & _
            " select  count(distinct car.car_id) cnt  , coalesce(admin.admin_name,'ไม่ระบุ') _admin_name   " & _
            " fROM license LEFT JOIN car on car.car_id = license.car_id  " & _
            " LEFT JOIN travel_group_car on travel_group_car.license_id = license.license_id " & _
            " LEFT JOIN travel_group on travel_group.group_id = travel_group_car.group_id " & _
            " left join admin on admin.admin_id = case when typeuser_id = 2 then travel_group.admin_id else license.admin_id  end " & _
            strWhere & " group by coalesce(admin.admin_name,'ไม่ระบุ')   ) dt"

            Dim strOrder_old As String = "select row_number() OVER ( PARTITION BY _admin_name  ORDER BY cnt DESC) AS i , _admin_name , _border_nameth_in  from ( " & _
            " select  count(distinct license.license_id) cnt  , coalesce(admin.admin_name,'ไม่ระบุ') _admin_name, coalesce(border_checkin.border_nameth,'ไม่ระบุ') as _border_nameth_in " & _
            " fROM license_old license left join admin on admin.admin_id = license.admin_id " & _
            " left join border_check border_checkin on border_checkin.border_id = license.checkin_id " & _
            strWhere_old & " group by coalesce(admin.admin_name,'ไม่ระบุ') , coalesce(border_checkin.border_nameth,'ไม่ระบุ')  ) dt "

            Dim strOrderGroup_old As String = " select row_number() OVER ( ORDER BY cnt DESC) AS i , _admin_name from ( " & _
            " select  count(distinct license.license_id) cnt  , coalesce(admin.admin_name,'ไม่ระบุ') _admin_name   " & _
            " fROM license_old license left join admin on admin.admin_id = license.admin_id " & _
            strWhere_old & " group by coalesce(admin.admin_name,'ไม่ระบุ')   ) dt"

            Dim str As String = " select dtOrder.i as ord , dtOrderGroup.i as ord_group , count(distinct car.car_id) cnt , count(distinct license.driver_id ) cntdriver " & _
                   " , coalesce(admin.admin_name,'ไม่ระบุ') admin_name, coalesce(border_checkin.border_nameth,'ไม่ระบุ') as border_nameth_in, coalesce(border_checkout.border_nameth,'ไม่ระบุ')  as border_nameth_out " & _
                   " , coalesce(car.country_car,'ไม่ระบุ') countries , coalesce(type_car.type_name_th,'ไม่ระบุ') || '(' type_car " & _
                   " , case when typeuser_id = 2 then travel_group.start_date else license.start_date end as start_date " & _
                   " , case when typeuser_id = 2 then travel_group.exp_date else license.exp_date end as exp_date  " & _
                   " , case when typeuser_id = 2 then name_company else CAST(driver.prename || ' ' || driver.name || ' ' || driver.surname as varchar) end  as driver_name " & _
                   " fROM license LEFT JOIN car on car.car_id = license.car_id  " & _
                   ...
                   " left join ( " & strOrder & " ) dtOrder on dtOrder._admin_name = coalesce(admin.admin_name,'ไม่ระบุ') and dtOrder._border_nameth_in = coalesce(border_checkin.border_nameth,'ไม่ระบุ') " & _
                   " left join ( " & strOrderGroup & " ) dtOrderGroup on dtOrderGroup._admin_name = coalesce(admin.admin_name,'ไม่ระบุ') " & _
                    strWhere
            str = str & " group by coalesce(admin.admin_name,'ไม่ระบุ') , coalesce(border_checkin.border_nameth,'ไม่ระบุ') , coalesce(border_checkout.border_nameth,'ไม่ระบุ') " & _
                        " , coalesce(car.country_car,'ไม่ระบุ') , coalesce(type_car.type_name_th,'ไม่ระบุ') , case when typeuser_id = 2 then travel_group.start_date else license.start_date end " & _
                        " , case when typeuser_id = 2 then travel_group.exp_date else license.exp_date end , case when typeuser_id = 2 then name_company else CAST(driver.prename || ' ' || driver.name || ' ' || driver.surname as varchar) end " & _
                        " , dtOrder.i , dtOrderGroup.i "

            str = str & "union all" & _
                " select dtOrder.i as ord , dtOrderGroup.i as ord_group , count(distinct license.license_id) cnt , count(distinct license.license_id ) cntdriver " & _
                   " , coalesce(admin.admin_name,'ไม่ระบุ') admin_name, coalesce(border_checkin.border_nameth,'ไม่ระบุ') as border_nameth_in, coalesce(border_checkout.border_nameth,'ไม่ระบุ')  as border_nameth_out " & _
                   " , coalesce(license.country_car,'ไม่ระบุ') countries , coalesce(type_car.type_name_th,'ไม่ระบุ') || '(' type_car " & _
                   " , license.start_date, license.exp_date, name_company as driver_name " & _
                   ...
                   " left join user_travel on user_travel.user_id = license.travel_id " & _
                   " left join ( " & strOrder_old & " ) dtOrder on dtOrder._admin_name = coalesce(admin.admin_name,'ไม่ระบุ') and dtOrder._border_nameth_in = coalesce(border_checkin.border_nameth,'ไม่ระบุ') " & _
                   " left join ( " & strOrderGroup_old & " ) dtOrderGroup on dtOrderGroup._admin_name = coalesce(admin.admin_name,'ไม่ระบุ') " & _
                    strWhere_old
            str = str & " group by coalesce(admin.admin_name,'ไม่ระบุ') , coalesce(border_checkin.border_nameth,'ไม่ระบุ') , coalesce(border_checkout.border_nameth,'ไม่ระบุ') " & _
                        " , coalesce(license.country_car,'ไม่ระบุ') , coalesce(type_car.type_name_th,'ไม่ระบุ') , license.start_date, license.exp_date , name_company " & _
                        " , dtOrder.i , dtOrderGroup.i "
            dtReport = db.getDataTable(str, "report")
...
#Region "Export ไฟล์"
    ...
    Private Function getQuery() As String
        ...
        If Request.QueryString("type") = 1 Then
            ''ประจำเดือน"
            If typeuser_id = 2 Then
                Dim _startdate As String = " (EXTRACT(year FROM travel_group.start_date) :: text || to_char(EXTRACT(MONTH FROM travel_group.start_date) :: integer,'FM09')  :: text) :: integer "
                Dim _expdate As String = " (EXTRACT(year FROM travel_group.exp_date) :: text || to_char(EXTRACT(MONTH FROM travel_group.exp_date) :: integer,'FM09')  :: text) :: integer "
                Dim schdate As String = ddlSchyear.SelectedValue - 543 & Format(CDbl(ddlSchMonth.SelectedValue), "00")
                strWhere = strWhere & " and ( " & schdate & " between " & _startdate & " and " & _expdate & " ) "
            Else
                Dim _startdate As String = " (EXTRACT(year FROM license.start_date) :: text || to_char(EXTRACT(MONTH FROM license.start_date) :: integer,'FM09')  :: text) :: integer "
                Dim _expdate As String = " (EXTRACT(year FROM license.exp_date) :: text || to_char(EXTRACT(MONTH FROM license.exp_date) :: integer,'FM09')  :: text) :: integer "
                Dim schdate As String = ddlSchyear.SelectedValue - 543 & Format(CDbl(ddlSchMonth.SelectedValue), "00")
                strWhere = strWhere & " and ( " & schdate & " between " & _startdate & " and " & _expdate & " ) "
            End If
            strWhere = strWhere & " and license.status_id = 5 "
        ...
        Dim pQuery As String = " select row_number() OVER ( ORDER BY to_char(receipt_date, 'MM-DD-YYYY'), case when typeuser_id = 2 then to_char(travel_group.start_date, 'MM-DD-YYYY') else to_char(license.start_date, 'MM-DD-YYYY') end  asc) AS i,coalesce(admin.admin_name,'ไม่ระบุ') admin_name, to_char(receipt_date, 'DD-MM-YYYY') as receipt_date, license.license_no " & _
