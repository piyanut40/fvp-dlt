...
Partial Class Admin_BorderCheck
    ...
    Private Sub loaddata()
        ...
        Try
            ...
            Dim str As String = " SELECT ROW_NUMBER() OVER (order by border_nameth ) as num , border_id , border_nameth , border_nameen , province.prov_th , CAST('BorderCheckMgt.aspx?id=' || border_id as varchar ) as urledit " & _
                " FROM border_check left join province on province.prov_code = border_check.prov_code "
            If txtName.Text <> "" Then
                str = str & "WHERE border_nameth like '%" & txtName.Text & "%' "
            End If
            str = str & " order by border_nameth "
    ...
    Protected Sub GridView1_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles gvMain.RowDataBound
        ...
        ElseIf e.Row.RowType = DataControlRowType.DataRow Then
            ...
            Dim lblborder_id As Label = e.Row.Cells(0).FindControl("lblborder_id")
            lnkDel.Attributes.Add("onclick", "javascript:DelData(" & lblborder_id.Text & ");")
        End If
    End Sub
    ...
    Protected Sub lnkAdd_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles lnkAdd.Click
        Response.Redirect("BorderCheckMgt.aspx")

    End Sub

    Protected Sub BtnDelete_Click(sender As Object, e As System.EventArgs) Handles BtnDelete.Click
        Dim dbConnect As New DBConnect
        Dim IsCheck As Integer = dbConnect.executeScalar(" SELECT count(license_id) FROM public.license where checkin_id = " & HidDel_ID.Value & " or checkout_id = " & HidDel_ID.Value)
        If IsCheck > 0 Then
            ScriptManager.RegisterStartupScript(Page, GetType(Page), "AlertScript", "alert('ไม่สามารถลบข้อมูลได้ เนื่องจากมีการเรียกใช้ข้อมูลด่านดังกล่าวแล้ว');", True)
        Else
            ...
            Try
                ...
                strdel = "DELETE from border_check WHERE border_id = " & HidDel_ID.Value
