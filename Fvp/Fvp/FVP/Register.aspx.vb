...
Partial Class Register
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Page.IsPostBack = False Then
            If Request.QueryString("token") <> "" And Request.QueryString("regis") <> "" Then
                Dim strSQL = " select license_id , act_id , license_id , driver_id , car_id , fname , lname , email " & _
                             " from license WHERE token = '" & Request.QueryString("token") & "' and status_id is null "
                Dim drTable As DataTable = dbconnect.ReadDataTable(strSQL)
                If drTable.Rows.Count > 0 Then
                    Session("license_id") = drTable.Rows(0).Item("license_id")
                    Session("driver_id") = drTable.Rows(0).Item("driver_id")
                    Session("car_id") = drTable.Rows(0).Item("car_id")
                    Session("act_id") = drTable.Rows(0).Item("act_id")
                    txtFname.Text = drTable.Rows(0).Item("fname")
                    txtLname.Text = drTable.Rows(0).Item("lname")
                    txtLicenseEmail.Text = drTable.Rows(0).Item("email")
                    txtEmail.Text = drTable.Rows(0).Item("email")
            ...
            If Request.QueryString("regis") = "1" Then
                ...
                If Request.QueryString("country") = "1" Then
                    ...
                    ddlOwnerCountry.SelectedValue = "Myanmar"
                Else
                    ...
                    ddlOwnerCountry.SelectedValue = "Cambodia"
                End If
            ElseIf Request.QueryString("regis") = "2" Then
                ddlCountryCar.Items.Add(New ListItem("Myanmar", "Myanmar"))
                ddlCountryCar.Items.Add(New ListItem("Cambodia", "Cambodia"))
            ...
            ElseIf Request.QueryString("regis") = "3" Then
                ...
                ddlOwnerCountry.SelectedValue = "Lao"
            ...
            ElseIf Request.QueryString("regis") = "4" Then
                ...
                If Request.QueryString("country") = "1" Then
                    ...
                    ddlOwnerCountry.SelectedValue = "Malaysia"
                Else
                    ...
                    ddlOwnerCountry.SelectedValue = "Singapore"
                End If
                receive.Visible = False
            End If
    ...
    Protected Sub btnNext1_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnNext1.Click
        ...
        Dim ptoken = ""
        Try
            If txtFname.Text = "" Or txtLname.Text = "" Or txtLicenseEmail.Text = "" Then
                ScriptManager.RegisterStartupScript(Page, Me.GetType(), "ScriptError", "fromCheck();", True)
            Else

                Dim strCheckUser As String = "select username from user_cus WHERE username like '%" & txtLicenseEmail.Text & "%'"
                dt = dbConnect.ReadDataTable(strCheckUser)
                If dt.Rows.Count > 0 Then

                    ScriptManager.RegisterStartupScript(Page, Me.GetType(), "Script", "alertalready();", True)

                Else
                    TableCommand.Rows.Clear()
                    TableCommand.Rows.Add("name", NpgsqlTypes.NpgsqlDbType.Varchar, "")
                    Dim rtndriver = dbConnect.InsertDataTableReturn(TableCommand, "driver", "driver_id")

                    TableCommand.Rows.Clear()
                    TableCommand.Rows.Add("plate", NpgsqlTypes.NpgsqlDbType.Varchar, "")
                    TableCommand.Rows.Add("typecar_id", NpgsqlTypes.NpgsqlDbType.Varchar, Request.QueryString("type"))
                    Dim rtncar = dbConnect.InsertDataTableReturn(TableCommand, "car", "car_id")

                    TableCommand.Rows.Clear()
                    TableCommand.Rows.Add("act_name", NpgsqlTypes.NpgsqlDbType.Varchar, "")
                    Dim rtnact = dbConnect.InsertDataTableReturn(TableCommand, "act", "act_id")

                    TableCommand.Rows.Clear()
                    TableCommand.Rows.Add("car_id", NpgsqlTypes.NpgsqlDbType.Integer, rtncar)
                    TableCommand.Rows.Add("driver_id", NpgsqlTypes.NpgsqlDbType.Integer, rtndriver)
                    TableCommand.Rows.Add("act_id", NpgsqlTypes.NpgsqlDbType.Integer, rtnact)
                    If Request.QueryString("regis") = 1 Or Request.QueryString("regis") = 2 Then
                        If Request.QueryString("group") = 1 Then
                            TableCommand.Rows.Add("typegroup", NpgsqlTypes.NpgsqlDbType.Integer, 1)
                            TableCommand.Rows.Add("typeuser_id", NpgsqlTypes.NpgsqlDbType.Integer, Request.QueryString("regis"))
                        Else
                            TableCommand.Rows.Add("typegroup", NpgsqlTypes.NpgsqlDbType.Integer, 2)
                            TableCommand.Rows.Add("typeuser_id", NpgsqlTypes.NpgsqlDbType.Integer, 1)
                        End If
                    Else
                        TableCommand.Rows.Add("typeuser_id", NpgsqlTypes.NpgsqlDbType.Integer, Request.QueryString("regis"))
                        TableCommand.Rows.Add("typegroup", NpgsqlTypes.NpgsqlDbType.Integer, "")
                    End If
                    TableCommand.Rows.Add("email", NpgsqlTypes.NpgsqlDbType.Varchar, txtLicenseEmail.Text)
                    TableCommand.Rows.Add("fname", NpgsqlTypes.NpgsqlDbType.Varchar, txtFname.Text)
                    TableCommand.Rows.Add("lname", NpgsqlTypes.NpgsqlDbType.Varchar, txtLname.Text)
                    Dim rtnliencse = dbConnect.InsertDataTableReturn(TableCommand, "license", "license_id")
                    ptoken = dbConnect.Token(rtnliencse, rtncar)
                    TableCommand.Rows.Clear()
                    TableCommand.Rows.Add("token", NpgsqlTypes.NpgsqlDbType.Varchar, ptoken)
                    Dim check = dbConnect.UpdateDataTable(TableCommand, "license", "WHERE license_id = " & rtnliencse)
                    Dim url = Request.QueryString("regis")
                    If check = "" Then
                        If Request.QueryString("country") = "1" Then
                            url = url & "&country=" & Request.QueryString("country")
                        Else

                        End If

                        Dim name As String = txtFname.Text & " " & txtLname.Text
                        Send.EmailRegis(txtLicenseEmail.Text, name, ptoken, url)
                        Dim sendmail As String = " Send email success , Please check you email "
                        ScriptManager.RegisterStartupScript(Page, Me.GetType(), "ScriptSendmail", " NewAlert('" & sendmail & "'); ", True)
                    Else
                        ScriptManager.RegisterStartupScript(Page, Me.GetType(), "ScriptError", "Error();", True)
                    End If
    ...
    Protected Sub btnNext4_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnNext4.Click
        If txtBrands.Text = "" Or txtLicenseCar.Text = "" Or txtEnginCap.Text = "" Or txtWeight.Text = "" Then
            ScriptManager.RegisterStartupScript(Page, Me.GetType(), "ScriptError", "fromCheck();", True)
            ScriptManager.RegisterStartupScript(Page, Me.GetType(), "Script3", "tab4();", True)
        Else
            If Session("car_id") Then
                ...
                Try
                    ...
                    If dread.Read Then
                        ...
                        Else
                            cmd.Parameters.Add("authorize_car", NpgsqlTypes.NpgsqlDbType.Varchar).Value = "NULL"
                        End If
    ...
    Protected Sub btnLoaddata_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnLoaddata.Click
        ...
        Try
            ...
            Dim strHistory = " SELECT driver.driver_id , car.car_id, license_id , act.act_id  , countries , driver.prename , driver.national , driver.name , " & _
                             " driver.surname , driver.passport_no, driver.passport_expire , driver.birthday , gender , address , passport_photo , licensedriver_photo , license_expire , idcard_no ,  " & _
    ...
    Protected Sub btnSubmit_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSubmit.Click
        Try
            Send.Email(txtLicenseEmail.Text, txtFname.Text & " " & txtLname.Text)
        Catch ex As Exception
    ...
    Protected Sub Provarea_Delete(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.CommandEventArgs)
        ...
        ElseIf Request.QueryString("regis") = 2 And tbProvice.Rows.Count = 1 And e.CommandArgument <> 0 Then
            ScriptManager.RegisterStartupScript(Page, Me.GetType(), "ScriptBack", "alert('Do not delete because there must be at least 1 province.')", True)
        Else
