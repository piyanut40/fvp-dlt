...
Partial Class EditUser
    ...
    Private Sub savedata()
        ...
        Try
            ...
            cmd.CommandText = "select username , password from user_cus WHERE user_id = '" & Session("user_id") & "' and username = '" & Session("email") & "' "
            ...
            If dr.Read Then
                ...
                If txtNewPass.Text <> txtNewPassCon.Text Then
                    Dim alert As String = "alert('New password not match');"
                ...
                ElseIf dbConnect.password(lblUser.Text, txtOldPass.Text).ToString <> old Then
                    Dim alert As String = "alert('Current password incorrect');"
                    ScriptManager.RegisterStartupScript(Me, GetType(String), "Script", alert, True)
                    txtOldPass.Text = ""
                    txtNewPass.Text = ""
                    txtNewPassCon.Text = ""
                Else
                    ...
                    cmd.CommandText = "Update user_cus set password=:password where username = '" & lblUser.Text & "'"
                    cmd.Parameters.Clear()
                    cmd.Parameters.Add("password", NpgsqlTypes.NpgsqlDbType.Varchar).Value = newpass
                    cmd.ExecuteNonQuery()
                    Dim alert As String = "alert('Save new password Success');"
                    ScriptManager.RegisterStartupScript(Me, GetType(String), "Script", alert, True)
                    txtOldPass.Text = ""
                    txtNewPass.Text = ""
                    txtNewPassCon.Text = ""
