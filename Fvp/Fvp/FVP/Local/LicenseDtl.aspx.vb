...
Partial Class Local_LicenseDtl
    ...
    Private Sub LoadData()
        ...
        Try
            ...
            strsql = "SELECT * FROM ( SELECT driver.*, car.*, act.*, license.license_id , token , CAST(driver.prename || ' ' || driver.name || ' ' || driver.surname as varchar) as driver_name " & _
                ...
                " , type_name as typecar_en, model as models, status_th , (select string_agg(prov_en , ',') from area LEFT JOIN province on area.prov_code = province.prov_code  WHERE license_id = license.license_id) as provarea " '& _


            strsql = strsql & " , spare_1.license_exp_date as spare_1_license_exp_date , spare_1.passport_expire as spare_1_passport_expire " & _
                            " , spare_2.license_exp_date as spare_2_license_exp_date , spare_2.passport_expire as spare_2_passport_expire  "
            ...
            strsql = strsql & " , spare_1.email as spare_1_email , spare_1.passport_photo as spare_1_passport_photo , spare_1.licensedriver_photo as spare_1_licensedriver_photo " & _
                              " , spare_2.email as spare_2_email , spare_2.passport_photo as spare_2_passport_photo , spare_2.licensedriver_photo as spare_2_licensedriver_photo "
            ...
            If dr.Read Then
                ...
                If Request.QueryString("rt") = 2 Then
                    ...
                    If Not dr("group_start") Is DBNull.Value Then
                        lblstartdategroup.Text = dr("group_start")
                    End If

                    If Not dr("group_exp") Is DBNull.Value Then
                        lblexpdategroup.Text = dr("group_exp")
                    End If
            ...
            If Request.QueryString("rt") = 2 Then
                Dim qryGuide As String = " select CAST(user_prename || ' '|| user_name || ' '|| user_lastname as varchar) as nameguide from user_travel_group " & _
                                  " WHERE group_id in  (select travel_group_car.group_id  from user_travel_group " & _
                                  " LEFT JOIN travel_group_car on travel_group_car.group_id = user_travel_group.group_id " & _
                                  " LEFT JOIN license on license.license_id = travel_group_car.license_id " & _
                                  " WHERE license.token = '" & Request.QueryString("token") & "') "

                Dim tbguide As DataTable = dbConnect.getDataTable(qryGuide, "tbguide")

                gvguide.DataSource = tbguide
                gvguide.DataBind()
